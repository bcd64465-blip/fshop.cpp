...
void unpackfsh(path const &out, path const &in) {
    ...
    fsh.ForAllImages([&](ea::FshImage &image) {
        ...
        string fshNameFinal = fshName;
        if (nameCount.find(fshName) != nameCount.end()) {
            nameCount[fshName]++;
            fshNameFinal = fshName + "_" + to_string(nameCount[fshName]);
        } else {
            nameCount[fshName] = 0;
        }
        image.WriteToFile(out.parent_path() / (fshNameFinal + "." + options().fshUnpackImageFormat), globalVars().fshUnpackImageFormat);
    });
}
